...
function setupSplashCursor(
  ...
  class PointerPrototype {
    ...
    const { gl, ext } = getWebGLContext(canvasElement);
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      if (isWebGL2) {
        ...
        formatRGBA = getSupportedFormat(gl2, gl2.RGBA16F, gl2.RGBA, texType);
        formatRG = getSupportedFormat(gl2, gl2.RG16F, gl2.RG, texType);
        formatR = getSupportedFormat(gl2, gl2.R16F, gl2.RED, texType);
      } else {
        const texType = (halfFloatTexType as number | null) ?? gl.UNSIGNED_BYTE;
        formatRGBA = getSupportedFormat(gl, gl.RGBA, gl.RGBA, texType);
        formatRG = getSupportedFormat(gl, gl.RGBA, gl.RGBA, texType);
        formatR = getSupportedFormat(gl, gl.RGBA, gl.RGBA, texType);
      }
    ...
    function splat(
      ...
      if (!velocity || !dye || !canvasElement) return;
      ...
      const colorR = Array.isArray(color) ? color[0] : color.r;
      const colorG = Array.isArray(color) ? color[1] : color.g;
      const colorB = Array.isArray(color) ? color[2] : color.b;
      gl.uniform3f(splatProgram.uniforms.color, colorR, colorG, colorB);
      blit(dye.write);
      dye.swap();
    }

    function correctRadius(radius: number): number {
      if (!canvasElement) return radius;
